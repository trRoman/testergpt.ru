...
function evaluateAnswer(questionId: number, rawAnswer: string): boolean {
	...
	if (questionId === 7) {
		...
		const mentionsElectricTrain =
			answer.includes("электропоезд") || answer.includes("электро") || answer.includes("электричк");
	...
	if (questionId === 9) {
		...
		const widowLogic =
			...
			(answer.includes("умер") || answer.includes("мертв") || answer.includes("покой") || answer.includes("не может"));
